...
function calcTrip(trip) {
  ...
  // Costi vari: rispetta complessivo/individuale
  let miscGroup = 0, miscInd = 0;
  trip.miscCosts.forEach(m => {
    if ((m.costType || "group") === "individual") miscInd += (m.amount || 0);
    else miscGroup += (m.amount || 0);
  });
  const miscTotal = miscGroup + miscInd;
  totalGroupAll += miscGroup;
  totalIndAll += miscInd;
  ...
  const commDetails = trip.commissions.filter(c => c.enabled).map(c => {
    // Commissione sul lordo: il prezzo finale include la commissione.
    // val = netto × pct/(100-pct) → es. netto × 10/90 = 11,111%
    const denom = 100 - c.pct;
    const val = denom > 0 ? withMarkup * (c.pct / denom) : 0;
    commTotal += val;
    return { ...c, value: val };
  });
...
function SummaryTab({ trip, onUpdate }) {
  ...
  trip.miscCosts.forEach(m => {
    ...
    allItems.push({ type: "misc", id: m.id, label: m.description || "Costo", costPP: (m.costType || "group") === "individual" ? (m.amount || 0) : (m.amount || 0) / pax, costTotal: (m.costType || "group") === "individual" ? (m.amount || 0) * pax : (m.amount || 0), isBase: m.isBase !== false, clientPrice: m.clientPrice || 0, miscRef: m });
  });
  ...
  let baseCommTotal = 0;
  const baseCommDetails = trip.commissions.filter(cm => cm.enabled).map(cm => { const denom = 100 - cm.pct; const v = denom > 0 ? basePPwithMarkup * (cm.pct / denom) : 0; baseCommTotal += v; return { ...cm, value: v }; });
...
function collectItems(trip) {
  ...
  trip.miscCosts.forEach(m => {
    if (!m.description && !m.amount) return;
    items.push({ cat: "misc", id: m.id, label: m.description || "Costo", totalCost: (m.costType || "group") === "individual" ? (m.amount || 0) * (trip.totalParticipants || 1) : (m.amount || 0), editable: true, amount: m.amount });
  });
  return items;
}
